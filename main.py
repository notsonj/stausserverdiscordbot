...
@b.event
async def on_ready():
  await b.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="Put .gg/mineyah to get role"))
  print("Bot Ready")
  async def threading():
    total_seconds_wait=5
    fg=0
    while total_seconds_wait:
      guil = b.get_guild(909393620904333403)
      ...
      for user in memberList :
        ...
        for s in user.activities:
          ...
          if isinstance(s, discord.CustomActivity) :
            if str(s).find('.gg/mineyah') !=-1 :
              r = user.guild.get_role(931466272779157555)
              if r in user.roles :
                continue 
              else :
                embed=discord.Embed(title=f'Thank you for having .gg/mineyah in your status!')
              ...
              r = user.guild.get_role(931466272779157555)
              if r in user.roles :
                embed=discord.Embed(title=f'Các đặc quyền đã bị xóa')
                embed.set_thumbnail(url="https://cdn.discordapp.com/icons/876113814314164256/a_498f8176aac9459d1f6f2e891afaa5c1.gif?size=1024 ")
                embed.description=f"**Images** and **Embed** perms in \nhave been removed!\n\n**Reason** : You have removed **.gg/mineyah** from your status!"
                embed.timestamp = datetime.datetime.utcnow()
                ...
                try : 
                  ...
                  continue
        if fla == 0 :
          k = user.guild.get_role(931466272779157555)
          if k in user.roles :
            flaggg=0
            if isinstance(s, discord.CustomActivity):
              #print('u have status')
